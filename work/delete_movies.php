...
<body>
                ...
                ?>
                  ...
                  <iframe width="100%" height="140" class="i-frame"
                    src="<?=$mrow['movie_url']?>">
                        ...
                        <a class="span-medit" href="javascript: void(0)" onclick="delete_movie(<?=$mrow['movie_id']?>);">Delete</a>
                      </div>
                    <?php }
                  ?>
                </div>
    ...
    <script>
      ...
      function delete_movie(val){
        var r = confirm("Do You Want To Delete This Movie\nClick Ok To Continue.");
        if(r == true){
          $.ajax({
               type: 'post',
               url : 'ajax_delete_movie.php',
               data: {
                  mid : val
               },
               success: function(resp){
                  if(resp == 1){
                    alert("Movie Deleted");
                     window.location = 'delete_movies.php';
                  }
               }
            });
        }
