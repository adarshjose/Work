						...
						while($row = mysqli_fetch_array($res)){
							$catid = $row['cat_id'];
		          			...
		          			<?php
		          				...
		          				$selm = "SELECT $tbl_movie.movie_id, $tbl_movie.movie_name, $tbl_movie.movie_url FROM $tbl_movie, $tbl_moviecatrel  WHERE movie_status = 'active' AND $tbl_movie.movie_id = $tbl_moviecatrel.movie_id AND $tbl_moviecatrel.cat_id = '$catid' $limit";
		          				$mres = mysqli_query($con, $selm);
		          				$num = mysqli_num_rows($mres);
								if($num > 0){
			          				while($mrow = mysqli_fetch_array($mres)){ ?>
			          					<div class="form-group col-md-3" align="center">
											<iframe width="220" height="140" class="i-frame"
												src="<?=$mrow['movie_url']?>">
											</iframe><br>
					          		...
					          		<span class="" align="center">No Movies Found</span>
					          	<?php }
		          				?>
		          			</div>
		          		<?php 
